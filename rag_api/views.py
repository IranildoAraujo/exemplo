...
from .serializers import IngestSerializer, QuerySerializer, RagResponseSerializer, FileUploadSerializer
from .utils import get_chroma_client, get_gemini_model, embed_text_gemini, initialize_gemini
...
try:
    ...
    # Considere como lidar com isso - a API pode não funcionar corretamente.

# rag_api/views.py

# ... (imports e outras partes da view) ...
        ...
        try:
            ...
            for doc in documents_data:
                ...
                if not metadata: # Se for None ou um dicionário vazio {}
                    logger.warning(f"Documento '{doc['id']}' sem metadados ou com metadados vazios. Usando default.")
            ...
            if not ids: # Segurança extra, caso documents_data esteja vazio (embora o serializer deva pegar)
                 return Response({"message": "Nenhum documento para processar."}, status=status.HTTP_200_OK)
            ...
            logger.info(f"Adicionando {len(ids)} documentos à coleção '{settings.CHROMA_COLLECTION_NAME}'...")
            collection.add(
                ids=ids,
            ...
            return Response({"message": f"{len(ids)} documentos adicionados com sucesso."}, status=status.HTTP_201_CREATED)

        except ConnectionError as e:
             logger.error(f"Erro de conexão com ChromaDB: {e}", exc_info=True)
             return Response({"error": "Não foi possível conectar ao ChromaDB."}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
        except ValueError as e: # Capturar especificamente o ValueError de metadados
            logger.error(f"Erro de validação durante a ingestão (possivelmente metadados): {e}", exc_info=True)
            # Retornar um erro mais específico para o cliente
            return Response({"error": f"Erro de validação de dados para ChromaDB: {e}"}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
        ...
        try:
            ...
            try:
                collection = chroma_client.get_collection(name=settings.CHROMA_COLLECTION_NAME)
                 ...
                 logger.error(f"Coleção ChromaDB '{settings.CHROMA_COLLECTION_NAME}' não encontrada: {e}")
                 return Response(
            ...
            results = collection.query(
                query_embeddings=[query_embedding], # A API espera uma lista de embeddings
                n_results=top_k,
                include=['documents'] # Incluir o texto dos documentos nos resultados
            )
            logger.info(f"Busca no ChromaDB concluída. Encontrados {len(results.get('documents', [[]])[0])} resultados.")


            retrieved_documents = results.get('documents', [[]])[0] # A estrutura é [[doc1, doc2,...]]

            if not retrieved_documents:
                logger.warning("Nenhum documento relevante encontrado no ChromaDB para a query.")
                ...
                context_string = "Nenhum contexto relevante encontrado na base de dados."
            else:
...
Resposta:"""
            ...
            return Response({"error": f"Erro interno de configuração: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
            logger.error(f"Erro inesperado durante a consulta RAG: {e}", exc_info=True)
            return Response({"error": f"Ocorreu um erro interno inesperado: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class FileUploadIngestView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not extracted_text or not extracted_text.strip():
            logger.warning(f"Nenhum texto extraído ou texto vazio para o arquivo '{original_filename}'.")
            return Response({"message": "Nenhum conteúdo de texto encontrado no arquivo.", "filename": original_filename}, status=status.HTTP_200_OK)

        logger.info(f"Texto extraído de '{original_filename}' (Tamanho: {len(extracted_text)} caracteres).")
        ...
        if not chunks:
             logger.warning(f"Não foram gerados chunks para o arquivo '{original_filename}'.")
             return Response({"message": "O texto extraído não gerou chunks processáveis.", "filename": original_filename}, status=status.HTTP_200_OK)
        ...
        metadatas = [{"source": original_filename, "chunk_index": i, "total_chunks": len(chunks)} for i in range(len(chunks))]

        try:
            # 4. Gerar Embeddings para os Chunks (em lote)
            logger.info(f"Gerando embeddings para {len(chunks)} chunks de '{original_filename}'...")
            ...
            logger.info(f"Adicionando {len(chunk_ids)} chunks à coleção '{settings.CHROMA_COLLECTION_NAME}'...")
            collection.add(
                ids=chunk_ids,
                embeddings=embeddings,
                documents=chunks,
            ...
            logger.info(f"Chunks do arquivo '{original_filename}' adicionados com sucesso.")
            ...
            return Response({
             ...
             logger.error(f"Erro de conexão com ChromaDB ao processar '{original_filename}': {e}", exc_info=True)
             return Response({"error": "Não foi possível conectar ao ChromaDB."}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
        except ValueError as e: # Capturar ValueErrors de validação ou outros
            logger.error(f"Erro de valor durante ingestão do arquivo '{original_filename}': {e}", exc_info=True)
            return Response({"error": f"Erro de validação ou processamento: {e}"}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error(f"Erro inesperado ao processar o arquivo '{original_filename}': {e}", exc_info=True)
            return Response({"error": f"Ocorreu um erro interno inesperado: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)