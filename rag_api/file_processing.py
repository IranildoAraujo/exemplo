...
# except ImportError:
#     odf_text = None
#     logging.warning("odfpy não instalado. Upload de ODT não funcionará.")
...
def extract_text_from_pdf_with_ocr(file_content: bytes, lang: str = 'por') -> str:
    ...
    try:
        if pypdf:
            ...
            if text: # Se extração direta funcionou, retorna
                 ...
                 return text
        logger.warning("Extração direta do PDF falhou ou retornou vazio. Tentando OCR...")
    ...
    if not pytesseract or not convert_from_bytes or not Image:
         raise ImportError("Bibliotecas necessárias para OCR (pytesseract, pdf2image, Pillow) não estão disponíveis.")
...
def extract_text_from_xlsx(file_content: bytes) -> str:
    ...
    text = ""
    try:
        workbook = openpyxl.load_workbook(io.BytesIO(file_content), data_only=True) # data_only=True pega valores, não fórmulas
...
#     """Extrai texto de um conteúdo de arquivo ODT em bytes."""
#     if not odf_text:
#         raise ImportError("Biblioteca odfpy é necessária para processar ODTs.")
#     text = ""
#     try:
...
#     except Exception as e:
#         logger.error(f"Erro ao extrair texto do ODT: {e}", exc_info=True)
#         # raise ValueError(f"Não foi possível processar o ODT: {e}") from e
#     return text.strip()
...
def extract_text_from_file(uploaded_file) -> tuple[str | None, str | None]:
    ...
    logger.info(f"Processando arquivo '{filename}' com extensão '{file_extension}' ({len(file_content)} bytes)")
...
# Implementação simples. Para produção, considere usar Langchain:
# from langchain.text_splitter import RecursiveCharacterTextSplitter

def simple_chunker(text: str, chunk_size: int = 1500, chunk_overlap: int = 150) -> list[str]:
    ...
    while start < len(text):
        ...
        if start < 0 : start = 0 # Evitar loop infinito com overlap grande
